...
fn part2(data: &mut Vec<usize>) -> usize {
    ...
    let mut counter: HashMap<isize, usize> = HashMap::from([(0, 1)]);
    for ad in data.iter().map(|&x| x as isize) {
        counter.insert(
            ad,
            counter.get(&(ad - 3)).unwrap_or(&0)
                + counter.get(&(ad - 2)).unwrap_or(&0)
                + counter.get(&(ad - 1)).unwrap_or(&0),
        );
    }
